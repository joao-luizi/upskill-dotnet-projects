...
function submitPesquisaForm(e) {
  e.preventDefault();
  const searchValue = pesquisa.buscador.value;
  console.log(searchValue);
  if (searchValue !== "") {
    pesquisaDb = [];
    for (let i = 0; i < bookDb.length; i++) {
      for (const key in bookDb[i]) {
        if (key === "titulo" || key === "autor" || key === "info") {
          if (bookDb[i][key].includes(searchValue)) {
            pesquisaDb.push(bookDb[i]);
            break;
          }
        }
      }
...
function initEventListeners() {
  pesquisa.addEventListener("submit", submitPesquisaForm);
